...
const expectedTables = [
  ...
  'bloque_institucional', 'medio_contacto'
]
...
async function main() {
  ...
  const [boardMembers, contactMethods, blocks, news, resources, pdfMetadata] = await Promise.all([
  ...
  if (!boardMembers || !contactMethods || !blocks || !news || !resources || !pdfMetadata) throw new Error('Faltan datos de desarrollo después de ejecutar la seed.')
  ...
  console.info(JSON.stringify({ tables: tables.length, boardMembers, contactMethods, blocks, news, resources, pdfStorage: pdfMetadata.storageKey }))
}
