...
describe('contrato de persistencia del MVP público', () => {
  it('mantiene el proveedor de migraciones y la representación de archivos como metadatos', async () => {
    const [lock, schema, migration, projectMigration] = await Promise.all([
      ...
      readFile(new URL('prisma/migrations/20260925064223_agregar_tabla_proyecto/migration.sql', root), 'utf8')
    ])
    ...
    expect(projectMigration).toContain('proyecto_id_autor_fkey')
    expect(projectMigration).toContain('proyecto_estado_creado_en_idx')
    expect(projectMigration).not.toMatch(/\bBYTEA\b|\bBLOB\b/i)
  })
})
