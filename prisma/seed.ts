...
async function main() {
  ...
  for (const permission of [
    ...
    const savedPermission = await prisma.permission.upsert({ where: { code: permission.code }, update: permission, create: permission })
    await prisma.rolePermission.upsert({
      where: { roleId_permissionId: { roleId: role.id, permissionId: savedPermission.id } },
      update: {},
      create: { roleId: role.id, permissionId: savedPermission.id }
    })
  }
